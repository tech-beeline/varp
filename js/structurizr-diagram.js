structurizr.ui.Diagram = function(id, diagramIsEditable, constructionCompleteCallback) {
    ...
    this.applyAutomaticLayout = function(rankDirection, rankSeparation, nodeSeparation, edgeSeparation, linkVertices) {
        try {
            ...
            // Calculate extra bottom padding for clusters from actual font sizes
            // so that DeploymentNode name + metadata text fits without overlapping
            var maxFontSize = 24;
            cells.forEach(function(cell) {
                if (cell._computedStyle && cell._computedStyle.fontSize) {
                    maxFontSize = Math.max(maxFontSize, cell._computedStyle.fontSize);
                }
            });
            var extraBottom = (maxFontSize * nameFontSizeDifferenceRatio) + (maxFontSize * metadataFontSizeDifferenceRatio) + 30;
            ...
            joint.layout.DirectedGraph.layout(graph, {
                ...
                clusterPadding: {
                    ...
                    bottom: clusterPadding + extraBottom
                }
            });
